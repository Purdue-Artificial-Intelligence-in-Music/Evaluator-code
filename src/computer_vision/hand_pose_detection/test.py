...
def main():
  ...
  # if using WINDOWS
#   gesture_options = GestureRecognizerOptions(
#     base_options=BaseOptions(model_asset_buffer = open('gesture_recognizer_custom.task', "rb").read()),
#     running_mode=VisionRunningMode.VIDEO)
  ...
  with mp_hands.Hands(
    ...
    output_file_name = "demo.avi"
    writer = cv2.VideoWriter(output_file_name, cv2.VideoWriter_fourcc(*"MJPG"), 12.5,(output_frame_length,output_frame_width)) # algo makes a frame every ~80ms = 12.5 fps
    while cap.isOpened():
      ...
      for result in YOLOresults:
        if len(result.obb.xyxyxyxy) > 0:
          ...
          box_one_coord = Point2D(box_one_coordinate_1[0].item(), box_one_coordinate_1[1].item())
          box_two_coord = Point2D(box_two_coordinate_2[0].item(), box_two_coordinate_2[1].item())
          box_three_coord = Point2D(box_three_coordinate_3[0].item(), box_three_coordinate_3[1].item())
          box_four_coord = Point2D(box_four_coordinate_4[0].item(), box_four_coordinate_4[1].item())

          # Print rounded and exact coords to show precision
        #   print("coord box one", round_coord_box_one)
        #   print("first coord = ", coord_box_one)

          # Prepare text
          text_one = "Bow OBB Coords:"
          text_coord1 = f"Coord 1: ({box_one_coord.x}, {box_one_coord.y})"
          text_coord2 = f"Coord 2: ({box_two_coord.x}, {box_two_coord.y})"
          text_coord3 = f"Coord 3: ({box_three_coord.x}, {box_three_coord.y})"
          text_coord4 = f"Coord 4: ({box_four_coord.x}, {box_four_coord.y})"
        ...
        if len(result.obb.xyxyxyxy) >= 2:
          ...
          cv2.putText(image, text_coord1, top_right_corner_coord1_2, cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 0, 0), 4)
          cv2.putText(image, text_coord2, top_right_corner_coord2_2, cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 0, 0), 4)
          cv2.putText(image, text_coord3, top_right_corner_coord3_2, cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 0, 0), 4)
          cv2.putText(image, text_coord4, top_right_corner_coord4_2, cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 0, 0), 4)
    ...
    # End video writing and video displaying
    cap.release()
    writer.release()
    cv2.destroyAllWindows()

    # Prints the file path address of the output file.
    print()
    print(output_file_name, "is saved to a file path address below:\n", os.path.abspath(output_file_name), "\n")
