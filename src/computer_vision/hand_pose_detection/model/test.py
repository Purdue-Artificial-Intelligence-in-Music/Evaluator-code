...
class Hands:
    ...
    def logging_csv(number, mode, handedness, frame_num, landmarks):
        ...
        #number/label is hardcoded for each posture (0 for normal, 1 for supination, 2 for pronation)
        number = 0
        csv_path = 'NormalPostureKeypointData.csv'
        ...
        if mode == 1:
            if number == 2:
                ...
                if handedness.classification[0].label[0:] == "Right" and handedness.classification[0].score >= 0.97 and landmark_list[8] > 0:
                    with open(csv_path, 'a', newline="") as f:
                        writer = csv.writer(f)
                        writer.writerow([number, frame_num, *landmark_list])
                        print("Logged data on frame " + str(frame_num))
            if number == 1:
        ...
        csv_path = 'elbow_training_data.csv'
        ...
        print(shoulder_elbow_dist_norm)
        print(hand_elbow_dist_norm)

        #0 for normal, 1 for low, 2 for high
        number = 2

        if mode == 1:
            with open(csv_path, 'a', newline="") as f:
                        writer = csv.writer(f)
                        writer.writerow([number, shoulder_elbow_dist_norm[0], shoulder_elbow_dist_norm[1], shoulder_elbow_dist_norm[2], hand_elbow_dist_norm[0], hand_elbow_dist_norm[1], hand_elbow_dist_norm[2], theta_rad_3d, shoulder_elbow_dist, hand_elbow_dist])
                        print("Logged data on frame " + str(frame_num))

        '''
        if mode == 1:
            if right_index.x < shoulder.x:
                global num_frames_high_elbow
                num_frames_high_elbow += 1
                if theta_deg_3d > 145:
                    global num_frames_detected
                    num_frames_detected += 1

        print(f"Frames with high elbow: {num_frames_high_elbow} Frames detected: {num_frames_detected}")
        '''
        ...
        if mode == 1:
            with open(csv_path, 'a', newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([frame_num, theta_deg_3d])
                    print("Logged data on frame " + str(frame_num))
        '''
        
        '''
        csv_path = 'ElbowTooHighInRangeDegrees.csv'

        #finds angle at elbow using x and y coordinates of shoulder, elbow, and hand
        shoulder = landmarks[11]
        elbow = landmarks[13]
        right_hand = landmarks[15]
        right_index = landmarks[19]
        ...
        a_3d = shoulder_3d - elbow_3d
        b_3d = right_hand_3d - elbow_3d

        cos_theta_3d = np.dot(a_3d, b_3d) / (np.linalg.norm(a_3d) * np.linalg.norm(b_3d))
        theta_rad_3d = np.arccos(np.clip(cos_theta_3d, -1.0, 1.0))
        theta_deg_3d = np.degrees(theta_rad_3d)
    ...
    def classify_elbow_posture(landmarks, debug_image):

        '''
        # initializes the coordinates of shoulder, elbow, right hand, and right index
        shoulder = landmarks[11]
        elbow = landmarks[13]
        right_hand = landmarks[15]
        right_index = landmarks[19]
        ...
        # finds angle at elbow using x, y, and z coordinates of shoulder, elbow, and hand
        elbow_shoulder_distance = shoulder_3d - elbow_3d
        hand_elbow_distance = right_hand_3d - elbow_3d

        cos_theta_3d = np.dot(elbow_shoulder_distance, hand_elbow_distance) / (np.linalg.norm(elbow_shoulder_distance) * np.linalg.norm(hand_elbow_distance))
        elbow_angle_radians = np.arccos(np.clip(cos_theta_3d, -1.0, 1.0))
        elbow_angle_degrees = np.degrees(elbow_angle_radians)

        print(elbow_angle_degrees)

        # checks if elbow angle is above threshold only if right index x-coordinate is to the left of shoulder x-coordinate
        threshold = 145

        ratio = ((elbow.x - shoulder.x)**2 + (elbow.y - shoulder.y)**2 + (elbow.z - shoulder.z)**2)**0.5 * 0.2

        if abs(elbow.y - right_hand.y) > ratio:
            if right_index.x < shoulder.x:
                if elbow_angle_degrees > threshold:
                    cv2.putText(debug_image, "Elbow Too High", (500, 300), cv2.FONT_HERSHEY_SIMPLEX, 2.4, (0, 0, 255), 4)
        else:
            cv2.putText(debug_image, "Elbow Too Low", (500, 300), cv2.FONT_HERSHEY_SIMPLEX, 2.4, (0, 0, 255), 4)
        '''
...
def main():
    ...
    model = YOLO(r'Evaluator-code\src\computer_vision\hand_pose_detection\bow_target.pt')  # Path to your model file
    ...
    video_file_path = 'right-elbow-too-high-2-2_kkRSShP6.mp4'
    ...
    #relative_csv_path_elbow = 'Evaluator-code\src\computer_vision\hand_pose_detection\model\keypoint_classifier\keypoint_classifier_label_elbow.csv'

    csv_path = os.path.join(base_directory, relative_csv_path)
    #csv_path_elbow =  os.path.join(base_directory, relative_csv_path_elbow)

    with open(csv_path, encoding='utf-8-sig') as f:
        keypoint_classifier_labels = csv.reader(f)
        keypoint_classifier_labels = [row[0] for row in keypoint_classifier_labels]
    
    #with open(csv_path_elbow, encoding='utf-8-sig') as f:
    #    keypoint_classifier_labels_elbow = csv.reader(f)
    #    keypoint_classifier_labels_elbow = [row[0] for row in keypoint_classifier_labels_elbow]
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks is not None:
                ...
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    Hands.logging_csv(number, mode, handedness, frame_count, landmarks)
                    
                    elbow_metrics = Hands.classify_elbow_posture(landmarks, debug_image)
                    '''
                    elbow_classification_id = elbow_classifier(elbow_metrics)

                    if elbow_classification_id == 0:
                        cv2.putText(debug_image, "Elbow Normal", (500, 300), cv2.FONT_HERSHEY_SIMPLEX, 2.4, (0, 0, 255), 4)
                    elif elbow_classification_id == 1:
                        cv2.putText(debug_image, "Elbow Too Low", (500, 300), cv2.FONT_HERSHEY_SIMPLEX, 2.4, (0, 0, 255), 4)
                    elif elbow_classification_id == 2:
                        cv2.putText(debug_image, "Elbow Too High", (500, 300), cv2.FONT_HERSHEY_SIMPLEX, 2.4, (0, 0, 255), 4)

                    #predicted_elbow_posture = keypoint_classifier_labels_elbow[elbow_classification_id]
                    #print(predicted_elbow_posture)
                    '''
                    # circles settings
